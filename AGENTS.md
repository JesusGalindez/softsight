...
| `npm run bench` | `npm run build:agent3d && node tools/bench.mjs` |
| `npm run bench:nightly` | `npm run build:agent3d && node tools/bench.mjs --nightly` |
...
<!-- generado: puertas -->
`npm run test:agents-md` · `npm run test:bands` · `npm run test:bind` · `npm run test:blend-contract` · `npm run test:boundaries` · `npm run test:bounds-tree` · `npm run test:bridge` · `npm run test:budgets` · `npm run test:bvh` · `npm run test:candidates` · `npm run test:capture-advice` · `npm run test:codes` · `npm run test:collision-qa` · `npm run test:colmap` · `npm run test:confidence` · `npm run test:contracts` · `npm run test:coverage` · `npm run test:determinism` · `npm run test:fixtures` · `npm run test:framing` · `npm run test:geometry` · `npm run test:glb-writer` · `npm run test:gltf-frame` · `npm run test:incremental` · `npm run test:lod-qa` · `npm run test:masks` · `npm run test:mcp` · `npm run test:measurement` · `npm run test:mesh-diff` · `npm run test:mesh-topology` · `npm run test:model-cache` · `npm run test:package-transport` · `npm run test:parity` · `npm run test:ply-binary` · `npm run test:preview-proxy` · `npm run test:producer-colmap` · `npm run test:producer-superficie` · `npm run test:production` · `npm run test:r0` · `npm run test:readiness` · `npm run test:recon-cache` · `npm run test:reconstruction` · `npm run test:repair` · `npm run test:report-r7` · `npm run test:resources` · `npm run test:rig` · `npm run test:screen` · `npm run test:self-intersection` · `npm run test:story` · `npm run test:summary` · `npm run test:text` · `npm run test:text-plan` · `npm run test:texture-qa` · `npm run test:uv-qa` · `npm run test:worker`
<!-- /generado: puertas -->
...
<!-- generado: banderas -->
`--model` · `--scene` · `--bvh` · `--patch` · `--dry-run` · `--undo` · `--control-poses` · `--sample` · `--frames` · `--fps` · `--skeleton` · `--bind` · `--audit-frames` · `--bvh-scale` · `--bvh-clip` · `--story` · `--reading-rate` · `--staging` · `--contrast-ratio` · `--out` · `--export` · `--save-scene` · `--inspect-only` · `--summary` · `--fields` · `--select` · `--select-where` · `--isolate` · `--audit-limit` · `--tile` · `--ground` · `--preview-max-triangles` · `--material-colors` · `--baseline` · `--baseline-report` · `--expect-size` · `--diff-samples` · `--diff-seed` · `--diff-max` · `--max-triangles` · `--max-parts` · `--require-watertight` · `--max-boundary-edges` · `--max-degenerate` · `--max-symmetry-error` · `--parity` · `--no-cache` · `--schema` · `--serve` · `--debug` · `--help`
<!-- /generado: banderas -->
